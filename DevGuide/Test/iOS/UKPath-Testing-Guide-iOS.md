...
This doc is scoped to iOS only, so it can run **concurrently** with [Android testing](../Android/UKPath-Testing-Guide-Android.md) and [Web testing](../Web/UKPath-Testing-Guide-Web.md) — all three share the same test-environment backend (see [Test Environment Guide](../UKPath-Test-Environment-Guide.md#1-the-test-environment)) and don't block each other.
...
iOS testing runs against the **test environment** API, same as Android — never production. See [Compliance Guide](../../Compliance/UKPath-Compliance-Guide.md#how-this-interacts-with-testing) for why `compliance_approvals` can be pre-seeded in this environment only.

## 2. Getting a Build Onto a Real iPhone Without a Local Mac

iOS builds require a Mac + Xcode locally, which is exactly what **CodeMagic** solves — it builds and code-signs the `.ipa` in the cloud (see [Test Environment Guide §4](../UKPath-Test-Environment-Guide.md#4-mobile-test-builds--codemagic)):
...
Per [Apple Guide §82](../../Dev/Apple/UKPath-Apple-Guide.md#82-ios-specific-requirements-to-give-the-developer), minimum supported version is iOS 15+.
...
- [ ] **Checkout flows** — confirm the [compliance gate](../../Compliance/UKPath-Compliance-Guide.md#code-enforced-compliance-gate) behaves identically to Android (this logic is server-side, so it should be, but verify the iOS client handles the `503` "not yet available" response gracefully rather than showing a raw error)

## 5. App Store Review Pre-Checks (Verify Early, Not Just Before Submission)

Per [Apple Guide §82](../../Dev/Apple/UKPath-Apple-Guide.md#82-ios-specific-requirements-to-give-the-developer):
...
- [ ] SF Pro font and iOS-native spacing/iconography used, not a reused Android layout (see [Apple Guide §8.3](../../Dev/Apple/UKPath-Apple-Guide.md#83-making-the-app-feel-native-without-duplicating-work))
...
Log defects against the module they belong to, and record the CodeMagic build number and TestFlight version tested, so a fix can be verified against the same build lineage — same convention as the [Android Testing Guide](../Android/UKPath-Testing-Guide-Android.md#7-reporting-results).
